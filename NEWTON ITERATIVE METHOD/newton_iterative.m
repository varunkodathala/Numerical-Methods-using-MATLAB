...
for n = 1:20
    g = diff(eqn); %Evaluates the differentiation of function
    g = subs(g,max_range);
    root(n) = (max_range - (subs(f,max_range)/g)); %Computes newton iterative equation
...
soln = subs(f,root(n)); % To Evaluate the solution
end
